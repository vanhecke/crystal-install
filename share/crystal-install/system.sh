...
{
	if [[ -n "$CRYSTAL_INSTALL_PKG_MANAGER" ]]; then
		package_manager="$CRYSTAL_INSTALL_PKG_MANAGER"
		return
	fi
...
#
# Maps os_platform/os_arch to Crystal's release archive platform string.
#
function resolve_crystal_platform()
{
	case "$os_platform" in
		Darwin)
			crystal_platform="darwin-universal"
			;;
		Linux)
			case "$os_arch" in
				x86_64)
					crystal_platform="linux-x86_64"
					;;
				aarch64|arm64)
					crystal_platform="linux-aarch64"
					;;
				*)
